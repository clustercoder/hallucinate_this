...
parser.add_argument("--use-langgraph", action="store_true", help="Use LangGraph-based orchestration instead of planner/executor flow")
...
if args.use_langgraph:
    # Build graph and run LangGraph orchestration (will use real ExecutorAgent if provided)
    graph = build_ctf_agent_graph()

    # Prepare executor backend/prompter for the graph
    executor_backend_cls = MODELS[config.executor.model]
    executor_backend = executor_backend_cls(Role.EXECUTOR, config.executor.model,
                                            environment.get_toolset(config.executor.toolset),
                                            keys[executor_backend_cls.NAME.upper()], config)
    ...
    try:
        # Start challenge container and environment
        challenge.start_challenge_container()
        environment.setup()

        state = {
            "task_text": challenge.canonical_name,
            "environment": environment,
            "challenge": challenge,
            "executor_prompter": executor_prompter,
            "executor_backend": executor_backend,
            "executor_max_rounds": config.executor.max_rounds,
        }
        result_state = graph.invoke(state)

        # Minimal logging: dump graph result
        logfile.parent.mkdir(parents=True, exist_ok=True)
        with logfile.open("w") as lf:
            json.dump({"langgraph_result": result_state}, lf, indent=2)
    finally:
        environment.teardown(None, None, None)
